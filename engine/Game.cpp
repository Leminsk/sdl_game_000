...
void Game::update() {
    ...
    for(auto& p : entity_distances) {
        ...
        if(cum_mov_vec.x >= 0) {
            if(curr_vec.x >= 0) { cum_mov_vec.x = std::max(cum_mov_vec.x, curr_vec.x); }
            else { cum_mov_vec.x += curr_vec.x; }
        } else {
            if(curr_vec.x < 0) { cum_mov_vec.x = std::min(cum_mov_vec.x, curr_vec.x); }
            else { cum_mov_vec.x += curr_vec.x; }
        }
        if(cum_mov_vec.y >= 0) {
            if(curr_vec.y >= 0) { cum_mov_vec.y = std::max(cum_mov_vec.y, curr_vec.y); }
