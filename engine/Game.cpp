...
Vector2D getTranslationDynamicVSTilesCollision(
    const Collider& dynamic_col, const std::vector<Entity*>& tiles, 
    const std::vector<std::pair<int,float>>& entity_distances, const Vector2D& prev_dynamic_pos
) {
    ...
    for(auto& p : entity_distances) {
        curr_vec = Collision::Collide(dynamic_col, tiles[p.first]->getComponent<Collider>(), p.second, prev_dynamic_pos);
...
void Game::init(const char* title, int width, int height, bool fullscreen) {
    ...
    player.addComponent<TransformComponent>(0.0f, 0.0f, 200.0f, 200.0f, 1.0);
...
void Game::update() {
    ...
    player_collider->transform->position = player_collider->transform->position + getTranslationDynamicVSTilesCollision(*player_collider, tiles, entity_distances, prev_player_pos);
