...
class KeyboardController : public Component {
    public:
        ...
        void update() override {
            ...
            if(keystates[SDL_SCANCODE_W]) { *vy = std::max(*vy - 1.0f, -5.0f); }
            if(keystates[SDL_SCANCODE_S]) { *vy = std::min(*vy + 1.0f,  5.0f); }
            if(keystates[SDL_SCANCODE_A]) { *vx = std::max(*vx - 1.0f, -5.0f); }
            if(keystates[SDL_SCANCODE_D]) { *vx = std::min(*vx + 1.0f,  5.0f);; }
